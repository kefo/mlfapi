    ...
    if ( $is-admin eq fn:false() or $login eq fn:false() ) then
        ...
        return
            if ($is-multiple-resources eq fn:true()) then
                ...
                let $existing-semtriples := $doc/mets:mets/mets:dmdSec[@ID="semtriples"]/mets:mdWrap[fn:last()]
                let $_ := xdmp:node-replace($existing-semtriples, $mdWrap-semtriples)
                ...
                return (xdmp:set-response-code(201, "201 Created"), ())
